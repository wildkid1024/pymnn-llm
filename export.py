...
class LLM(torch.nn.Module):
    ...
    def export_lm(self):
        model = self.lm
        hidden_states = torch.randn(3, 1, 4096)
    ...
    def export_block(self, block_id: int):
        ...
        if self.export_test:
            ...
            print("size:", original_outs[0].size(), original_outs[1].size())
            ort_session = ort.InferenceSession(onnx_model, providers=['CPUExecutionProvider'])
            inputs = {
...
if __name__ == '__main__':
    ...
    # python3 py-mnn-llm/export.py --path /public/Models/chatglm-6b --type=chatglm-6b --export_lm --export_test --export_path=py-mnn-llm/onnx
    # mnnconvert -f ONNX --modelFile embedding2.onnx --MNNModel embedding.mnn --fp16